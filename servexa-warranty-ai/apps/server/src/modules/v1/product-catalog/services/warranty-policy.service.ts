...
export class WarrantyPolicyService implements IWarrantyPolicyService {
  ...
  async findAll(query: FindAllWarrantyPoliciesInput) {
    const where: Prisma.WarrantyPolicyWhereInput = {
      ...
      ...(query.search
        ? {
            OR: [
              { coverageDescription: { contains: query.search, mode: 'insensitive' } },
              { termsConditions: { contains: query.search, mode: 'insensitive' } },
            ],
          }
        : {}),
      ...(query.effectiveOn
  ...
  async findOneById(warrantyPolicyId: string) {
    const found = await this.warrantyPolicyRepository.findOneById(warrantyPolicyId, {
      select: warrantyPolicyDetailSelect,
    })

    if (!found) {
      throw createOperationalError('Warranty policy not found', HTTP_RESPONSE_CODE.NOT_FOUND)
    }
  ...
  async create(input: CreateWarrantyPolicyDto) {
    ...
    if (input.categoryId) {
      const category = await this.categoryRepository.findOneById(input.categoryId, { select: { id: true } })
      if (!category) throw createOperationalError('Category not found', HTTP_RESPONSE_CODE.BAD_REQUEST)
    } else if (input.modelId) {
      const model = await this.modelRepository.findOneById(input.modelId, { select: { id: true } })
      if (!model) throw createOperationalError('Model not found', HTTP_RESPONSE_CODE.BAD_REQUEST)
    }
    ...
    if (input.status === 'active') {
      ...
      if (overlapping && overlapping.length > 0) {
        throw createOperationalError('Overlapping active warranty policy exists', HTTP_RESPONSE_CODE.CONFLICT)
      }
    }
  ...
  async update(warrantyPolicyId: string, input: ReplaceWarrantyPolicyDto | UpdateWarrantyPolicyDto) {
    const existing = (await this.warrantyPolicyRepository.findOneById(warrantyPolicyId)) as any
    ...
    if (
      ...
      throw createOperationalError('Exactly one of categoryId or modelId must be non-null', HTTP_RESPONSE_CODE.BAD_REQUEST)
    }

    // Check target existence if changed
    if (input.categoryId !== undefined && input.categoryId !== existing.categoryId && input.categoryId) {
      const category = await this.categoryRepository.findOneById(input.categoryId, { select: { id: true } })
      if (!category) throw createOperationalError('Category not found', HTTP_RESPONSE_CODE.BAD_REQUEST)
    } else if (input.modelId !== undefined && input.modelId !== existing.modelId && input.modelId) {
      const model = await this.modelRepository.findOneById(input.modelId, { select: { id: true } })
      if (!model) throw createOperationalError('Model not found', HTTP_RESPONSE_CODE.BAD_REQUEST)
    }
    ...
    if (input.modelId !== undefined) {
      data.model = input.modelId ? { connect: { id: input.modelId } } : { disconnect: true }
    }
    if (input.warrantyType !== undefined) data.warrantyType = input.warrantyType
    if (input.warrantyDurationMonths !== undefined) data.warrantyDurationMonths = input.warrantyDurationMonths
    if (input.coverageDescription !== undefined) data.coverageDescription = input.coverageDescription
    if (input.termsConditions !== undefined) data.termsConditions = input.termsConditions
  ...
  async delete(warrantyPolicyId: string) {
    const existing = await this.warrantyPolicyRepository.findOneById(warrantyPolicyId, { select: { id: true } })

    if (!existing) {
      throw createOperationalError('Warranty policy not found', HTTP_RESPONSE_CODE.NOT_FOUND)
    }

    const count = await prisma.productWarranty.count({ where: { warrantyPolicyId } })
    if (count > 0) {
  ...
  async resolve(input: ResolveWarrantyPolicyDto) {
    ...
    const resolved = await this.warrantyPolicyRepository.findActiveForResolve(target, input.warrantyType, date)

    if (!resolved) {
      throw createOperationalError('No matching active warranty policy found', HTTP_RESPONSE_CODE.NOT_FOUND)
    }
