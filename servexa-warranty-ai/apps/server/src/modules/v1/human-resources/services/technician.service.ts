...
export class TechnicianService implements ITechnicianService {
  constructor(private readonly technicianRepository: ITechnicianRepository = new TechnicianRepository()) {}
  ...
  async findOneById(technicianProfileId: string) {
    const found = (await this.technicianRepository.findOneById(technicianProfileId, {
      select: technicianSelect,
    })) as Record<string, unknown> | null
    if (!found) throw createOperationalError('Technician profile not found', HTTP_RESPONSE_CODE.NOT_FOUND)
    return found
  }
  ...
  async create(input: CreateTechnicianDto) {
    ...
    if (duplicate) {
      throw createOperationalError('Technician profile already exists for this user', HTTP_RESPONSE_CODE.CONFLICT)
    }
  ...
  async update(technicianProfileId: string, input: ReplaceTechnicianDto | UpdateTechnicianDto) {
    ...
    if (input.maxConcurrentCases !== undefined) data.maxConcurrentCases = input.maxConcurrentCases
    if (input.isAvailable !== undefined) data.isAvailable = input.isAvailable
    if (input.averageRepairTime !== undefined) data.averageRepairTime = input.averageRepairTime
    if (input.customerRating !== undefined) {
      data.customerRating =
        input.customerRating === null ? null : new Prisma.Decimal(input.customerRating)
    }
    if (input.completedCases !== undefined) data.completedCases = input.completedCases

    if (Object.keys(data).length === 0) {
      throw createOperationalError('No fields to update', HTTP_RESPONSE_CODE.BAD_REQUEST)
    }

    return this.technicianRepository.updateOneById(technicianProfileId, data, { select: technicianSelect })
  }
