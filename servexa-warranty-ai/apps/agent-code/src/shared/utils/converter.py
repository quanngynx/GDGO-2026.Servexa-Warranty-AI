...
def convert_to_vnd_currency(amount: int | float | str | Decimal) -> str:
    normalized_amount = str(amount).replace(',', '').strip()
    try:
        parsed_amount = Decimal(normalized_amount)
    except (InvalidOperation, ValueError, TypeError) as exc:
        raise ValueError(f'Invalid amount for VND conversion: {amount}') from exc

    rounded_amount = int(parsed_amount.quantize(Decimal('1')))
    formatted_amount = f'{rounded_amount:,}'.replace(',', '.')
